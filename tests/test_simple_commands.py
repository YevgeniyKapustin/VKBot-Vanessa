...
def test_complex_commands(chat_id, command):
    assert Response().response_definition(chat_id, command, 0, 0) in zmiysphrases


@pytest.mark.parametrize('chat_id, command', [(test_chat_id, 'д12')])
def test_dice(chat_id, command):
    response = int(Response().response_definition(chat_id, command, 0, 0).replace('🎲', ''))
...
def test_heroes_helper(chat_id, command):
    response = Response().response_definition(chat_id, command, 0, 0).replace('🎲 ', '')
