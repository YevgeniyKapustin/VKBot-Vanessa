...
from commands_logic import randomize, mute, add_command
from commands_logic.wiki import send_wiki_article
from actions import send_text, send_file, send_stick
...
class Response:
    ...
    def __check_special_commands(self):
        ...
        if self.msg == 'команды':
            ...
            return randomize.send_random_fraction(self.chat_id)

        elif self.msg == 'навык':
            return randomize.send_random_skill_position(self.chat_id)

        elif self.msg[:9] in ['что такое', 'кто такая', 'кто такой']:
            return send_wiki_article(self.chat_id, self.msg)

        elif self.msg[:3] == 'мут':
            return mute.Mute.shut_up(mute.Mute(), self.chat_id, self.msg,
                                     self.peer_id, self.event)

        elif self.msg[:6] == 'размут':
            return mute.Mute.redemption(mute.Mute(), self.chat_id, self.msg,
                                        self.event, self.peer_id)

        elif self.msg[:16] == 'добавить команду':
            return add_command.Commands.add_command(add_command.Commands(),
                                                    self.msg, self.chat_id,
                                                    self.event)

        elif self.msg[:15] == 'удалить команду':
            return add_command.Commands.remove_command(add_command.Commands(),
                                                       self.msg, self.chat_id)

        elif self.msg == 'абоба':
            return randomize.send_random_zmiysphrases(self.chat_id)

        elif self.msg == 'рарити':
            return randomize.send_random_rarity(self.chat_id)

        elif self.msg == 'статистика игроков':
            return send_text(self.chat_id, Cabbagesite.get_players_winrate())

        elif self.msg == 'статистика фракций':
            return send_text(self.chat_id, Cabbagesite.get_fractions_winrate())

        elif self.msg[:1] == 'д':
            return randomize.send_roll_dice(self.chat_id, self.msg)
